...
class Utils:
    ...
    def get_best_threshold(y_true, y_pred_probs, priority=None, min_recall=None, min_precision=None):
        ...
        if priority == "f1":
            ...
            if min_precision is not None:
                ...
                best_idx = valid_indices[np.argmax(recall[valid_indices])] if len(valid_indices) > 0 else np.argmax(precision)
            else:
    ...
    @staticmethod
    def online_validation_folds(dataset, n_folds=10, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15, min_samples=30):
        """
        Chia dữ liệu time series thành các fold với cửa sổ trượt, bao gồm train, validate, test.
        Thêm kiểm tra kích thước tối thiểu để tránh tập rỗng.
        """
        total_length = len(dataset)
        if total_length < min_samples:
            print(f"Dataset too small ({total_length} samples), skipping fold generation.")
            return [], [], []

        step_size = int(total_length / n_folds * train_ratio)
        train_sets, val_sets, test_sets = [], [], []

        for fold in range(n_folds):
            start_idx = fold * step_size
            train_end_idx = min(start_idx + int(total_length * train_ratio), total_length)
            val_end_idx = min(train_end_idx + int(total_length * val_ratio), total_length)
            test_end_idx = min(val_end_idx + int(total_length * test_ratio), total_length)

            train_set = dataset.iloc[start_idx:train_end_idx]
            val_set = dataset.iloc[train_end_idx:val_end_idx]
            test_set = dataset.iloc[val_end_idx:test_end_idx]

            # Kiểm tra kích thước tối thiểu cho mỗi tập
            if len(train_set) < min_samples or len(val_set) < min_samples or len(test_set) < min_samples:
                print(f"Fold {fold + 1} skipped: Insufficient samples (Train: {len(train_set)}, Val: {len(val_set)}, Test: {len(test_set)})")
                continue

            print(f"Fold {fold + 1}: Train {start_idx}-{train_end_idx}, Val {train_end_idx}-{val_end_idx}, "
                  f"Test {val_end_idx}-{test_end_idx}")
            train_sets.append(train_set)
            val_sets.append(val_set)
            test_sets.append(test_set)

        if not train_sets or not val_sets or not test_sets:
            print("No valid folds generated due to insufficient data.")
            return [], [], []
    ...
    def calculate_weighted_score(metrics, weights=None, threshold_upper=0.99, threshold_lower=0.7):
        if weights is None:
            weights = {"AUC": 0.2, "F1": 0.25, "accuracy": 0.0, "recall": 0.3, "precision": 0.1, "PR_AUC": 0.15}
