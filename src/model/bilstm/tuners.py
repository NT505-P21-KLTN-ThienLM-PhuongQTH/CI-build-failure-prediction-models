# src/model/stacked_bilstm/tuners.py
import numpy as np
...
from src.model.bilstm.model import construct_bilstm_model  # Update import
...
class BiLSTMWorker(Worker):
    def __init__(self, train_set, **kwargs):
        super().__init__(**kwargs)
        self.train_set = train_set

    def compute(self, config, budget, **kwargs):
        res = construct_bilstm_model(config, self.train_set)
        return {'loss': res['validation_loss'], 'info': {}}

def train_bilstm_with_hyperopt(network_params):
    if 'data' not in globals():
        raise ValueError("Global 'data' not set. Ensure evaluate_tuner sets it correctly.")
    res = construct_bilstm_model(network_params, globals()['data'])
...
def fn_bilstm_pso(drop_proba=0.01, nb_units=32, nb_epochs=2, nb_batch=4, nb_layers=1, optimizer=1, time_step=30):
...
def evaluate_tuner(tuner_option, train_set, pretrained_model_path=None):
    ...
    if tuner_option == "ga":
        ...
        best_params, best_model, entry_train, history = ga_runner.generate(
            all_possible_params, construct_bilstm_model, data,
            pretrained_model_path=pretrained_model_path
        )
    elif tuner_option == "tpe":
        param_space = {k: hp.choice(k, v) for k, v in all_possible_params.items()}
        trials = Trials()
        best = fmin(train_bilstm_with_hyperopt, param_space, algo=tpe.suggest, max_evals=CONFIG.get('MAX_EVAL'),
                    trials=trials)
        best_params = {k: all_possible_params[k][v] for k, v in best.items()}
        res = construct_bilstm_model(best_params, data, pretrained_model_path=pretrained_model_path)
        ...
        best_params, _, _ = optunity.maximize_structured(fn_bilstm_pso, params_PSO, num_evals=CONFIG.get('MAX_EVAL'))
        ...
        NS = hpns.NameServer(run_id="BiLSTM", host='127.0.0.1', port=None)
        NS.start()
        w = BiLSTMWorker(train_set=data, nameserver='127.0.0.1', run_id="BiLSTM")
        w.run(background=True)
        bohb = BOHB(configspace=config_space, run_id="BiLSTM", nameserver='127.0.0.1', min_budget=1,
        ...
        param_space = {k: hp.choice(k, v) for k, v in all_possible_params.items()}
        trials = Trials()
        best = fmin(train_bilstm_with_hyperopt, param_space, algo=rand.suggest,
