# src/model/stacked_bilstm/validation.py
import os
import random
import pandas as pd
import mlflow
...
from src.model.common.preprocess import test_preprocess, apply_smote, prepare_features  # Update import
from src.model.bilstm.tuners import evaluate_tuner, CONFIG  # Update import
...
MODEL_NAME = "Stacked-BiLSTM"  # Update model name
        ...
        for file_name, dataset in datasets.items():
            ...
            for fold_idx, (train_set, test_set) in enumerate(zip(train_sets, test_sets)):
                for iteration in range(1, CONFIG['NBR_REP'] + 1):
                    ...
                    entry_test, threshold = Utils.predict_lstm(current_model, X_test, y_test)  # Verify compatibility
                    entry_test.update({
                        "iter": iteration,
                        "proj": file_name,
                        "exp": fold_idx + 1,
    ...
    with mlflow.start_run(run_name="Cross_Project_Validation_Main", nested=True):
        ...
        with mlflow.start_run(run_name="Fine_Tune", nested=True):
                ...
                for file_name, test_set in all_datasets.items():
                    if test_set is not bellwether_dataset:
                        ...
                        with mlflow.start_run(run_name=f"fine_tune_{file_name}_iter{iteration}", nested=True):
                            ...
                            entry_fine_tune = construct_bilstm_model(fine_tune_params, test_set,
                            ...
                            entry_test, threshold = Utils.predict_lstm(fine_tuned_model, X_test, y_test)  # Verify compatibility
                            entry_test.update({
                                "iter": iteration,
                                "proj": file_name,
                                "exp": 1,
            ...
            if best_cross_project_info is not None:
                best_project = best_cross_project_info["params"]["project"]  # Fix key reference
                best_iteration = best_cross_project_info["params"]["iteration"]  # Fix key reference
                best_model_name = f"best_stacked_bilstm_cross_project_{best_project}_iter{best_iteration}"
                ...
                print(f"Best cross-project model registered as '{MODEL_NAME}', "
                      f"F1: {best_cross_project_info['metrics']['f1']}, "  # Fix key reference
                      f"Project: {best_project}, Iteration: {best_iteration}, "
                      f"Threshold: {best_cross_project_info['params']['threshold']}")  # Fix key reference