...
def convlstm_train_preprocess(dataset_train, time_step):
    cache_file = os.path.join(CACHE_DIR, f"train_cache_{time_step}_{hash(str(dataset_train.values.tobytes()))}.pkl")
    ...
    with open(cache_file, 'wb') as f:
        pickle.dump((X_train_grouped, y_train), f)
    return X_train_grouped, y_train


def convlstm_test_preprocess(dataset_train, dataset_test, time_step):
    cache_file = os.path.join(CACHE_DIR,
                              f"test_cache_{time_step}_{hash(str(dataset_train.values.tobytes()))}_{hash(str(dataset_test.values.tobytes()))}.pkl")
    if os.path.exists(cache_file):
        print(f"Loading cached data from {cache_file}")
        with open(cache_file, 'rb') as f:
            return pickle.load(f)
    ...
    with open(cache_file, 'wb') as f:
        pickle.dump((X_test_grouped, y_test), f)
