...
class GARunner:
    ...
    def _train_solution(self, solution: Any, fn_train: Callable, params_fn: Dict, index: int, pretrained_model_path=None, fine_tune=True) -> None:
        # Train a single solution and handle exceptions.
        try:
            solution.train_model(fn_train, params_fn, pretrained_model_path=pretrained_model_path, fine_tune=fine_tune)
            print(f"\nSolution {index} trained")
        except Exception as e:
            print(f"Error training solution {index}: {str(e)}")
            raise

    def train_population(self, population: List[Any], fn_train: Callable, params_fn: Dict, pretrained_model_path=None, fine_tune=True) -> None:
        ...
        with tqdm(total=len(population), desc="\nTraining Population") as pbar:
            with ThreadPoolExecutor(max_workers=len(population)) as executor:
                futures = [
                    executor.submit(self._train_solution, sol, fn_train, params_fn, i, pretrained_model_path, fine_tune)
    ...
    def generate(self, all_possible_params: Dict[str, List], fn_train: Callable, params_fn: Dict, pretrained_model_path=None, fine_tune=True) -> Tuple[Dict, Any, Dict, Any]:
        ...
        for generation in range(self.ga_params['max_generations']):
            ...
            self.train_population(population, fn_train, params_fn, pretrained_model_path, fine_tune)
